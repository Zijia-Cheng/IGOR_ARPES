...
// Version 1.24, July 19, 2017: HDF5LoadGroup uses /R=2 flag added in HDF5 XOP 1.24.

// Version 1.25, December 14, 2017: Fixed scaling of browser panel coordinates
// in SetPrefWindowCoords to fix issue on Windows where browser window size was incorrect.

// Version 1.26, May 3, 2019: Added code to prevent Igor8 from generating an error
// if the user clicks a control added by Igor9.
...
static Function LoadGroupButtonProc(ctrlName) : ButtonControl
	...
	if (V_value)									// Use LoadGroupsRecursively is checked?
		// The /R=2 flag was added in HDF5 XOP 1.24. It tells HDF5LoadGroup to load
		// a given group even if it was previously loaded because it appears in multiple
		// places in the same HDF5 file's directed graph. See HDF5LoadGroup operation
		// documentation for details.
		HDF5LoadGroup /O /R=2 /T /IMAG=1 :, bd.fileID, groupPath		// Requires HDF5 XOP 1.24 or later
	else
...
static Function GroupsListActionProc(s, bd) : ListboxControl
	...
	switch(s.eventCode)
		case 4:						// Cell selection
			int numRows = DimSize(s.listWave, 0);
			if (s.row<0 || s.row>=numRows)
				// Printf "Skipping cell selection in GroupsListActionProc, s.row=%d, numRows=%d\r", s.row, numRows
				break					// User pressed arrow key when the list is empty
			endif
...
static Function GroupAttributesListActionProc(s, bd) : ListboxControl
	...
	switch(s.eventCode)
		...
		case 4:						// Cell selection
			int numRows = DimSize(s.listWave, 0);
			if (s.row<0 || s.row>=numRows)
				// Printf "Skipping cell selection in GroupAttributesListActionProc, s.row=%d, numRows=%d\r", s.row, numRows
				break					// User pressed arrow key when the list is empty
			endif
...
static Function DatasetsListActionProc(s, bd) : ListboxControl
	...
	switch(s.eventCode)
		...
		case 4:						// Cell selection
			int numRows = DimSize(s.listWave, 0);
			if (s.row<0 || s.row>=numRows)
				// Printf "Skipping cell selection in DatasetsListActionProc, s.row=%d, numRows=%d\r", s.row, numRows
				break					// User pressed arrow key when the list is empty
			endif
...
static Function DatasetAttributesListActionProc(s, bd) : ListboxControl
	...
	switch(s.eventCode)
		...
		case 4:						// Cell selection
			int numRows = DimSize(s.listWave, 0);
			if (s.row<0 || s.row>=numRows)
				// Printf "Skipping cell selection in DatasetAttributesListActionProc, s.row=%d, numRows=%d\r", s.row, numRows
				break					// User pressed arrow key when the list is empty
			endif
			// Printf "Row=%d, column=%d\r", s.row, s.col
...
Function HDF5BrowserPrefCheckboxProc(ctrlName,checked) : CheckBoxControl
	...
	strswitch(ctrlName)
		...
		default:
			DoAlert 0, "This control requires Igor Pro 9.00 or later"
			break
	endswitch
	HDF5BrowserSavePackagePrefs(prefs)
End

static Function HDF5BrowserCheckboxProc(s)		// Added in Igor Pro 8.04
	STRUCT WMCheckboxAction &s
	
	// If the user opens an experiment saved by Igor9 or later in Igor8,
	// this function prevents Igor8 from generating an error if the user clicks
	// a checkbox added by Igor9, such as the Sort By Creation Order checkbox.
	
	return HDF5BrowserPrefCheckboxProc(s.ctrlName,s.checked)
End

static Function RecentFilesPopMenuProc(pa)		// Added in Igor Pro 8.04
	STRUCT WMPopupAction &pa
	
	// If the user opens an experiment saved by Igor9 or later in Igor8,
	// this function prevents Igor8 from generating an error if the user clicks
	// the Recent Files popup menu added by Igor9.
...
Function HDF5ResizeBrowser(browserName)
	...
	if (PanelResolution(browserName) == 72)
		...
		GetWindow $browserName wsize				// Not compatibility mode - we need points, not pixels
	endif
...
static Function SetPrefWindowCoords(windowName)
	...
	// by NewPanel.
	Variable scale = ScreenResolution / PanelResolution(windowName)	// HR, 2017-12-14, 1.25: Fixed scaling issue on Windows
