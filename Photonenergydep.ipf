...
Function photonenergydep_ALS(inputwave, outputname,startenergy,energyrange)
		...
		// Assuming the work function is around 4.5 
...
Function temp_dep_SSRL(inputwave, outputname,workfunction,photonenergy,offsetwave)
		//20220528 by CZJ
		
		// The input should be a three dimensional wave with dimension : 0 -> angle, 1-> energy, 2-> index .
		// output wave's name is the string outputname. Its a three dimensional wave
		//Assuming work function to be 4.5 when transfering angle to momentum
		
		Wave inputwave
		String outputname
		Variable workfunction,photonenergy
		wave offsetwave //This is the shift of each cut in the angle
		...
		for(i = 0; i< dimsize(inputwave,2);i++)
			...
			String ekcut = "en"+num2str(i)
			Duplicate /O /RMD = [][][i] inputwave , $ekcut // remember here
			Setscale /P x, (pi/180)*0.5123*sqrt(photonenergy - workfunction)*(dimoffset($ekcut,0)-offsetwave[i]), (pi/180)*0.5123*sqrt(photonenergy - workfunction)*dimdelta($ekcut,0), $ekcut	
			Setscale /P y, dimoffset($ekcut,1) - (photonenergy - workfunction),dimdelta($ekcut,1),$ekcut
